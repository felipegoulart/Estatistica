...
const calcFreqPercent = dados => {
  ...
  for (const i in dados.valoresAgrupados){
    const percentSi = ((dados.valoresAgrupados[i] / dados.vetorDados.length) * 100).toFixed(2)
...
const calculaMediaModaMediana = dados => {
  ...
  if(dados.tipoVar === 'continua') {
    for (const data in dados.valoresAgrupados){
